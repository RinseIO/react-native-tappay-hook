...
describe('TappayFP test(android)', () => {
  ...
  test('getGooglePayPrime() test', async () => {
    expect(await TappayFP.getGooglePayPrime('1', 'TWD')).toMatchObject({
      systemOS,
      tappaySDKVersion: 'mock',
  ...
  test('getApplePayPrime() test', async () => {
    // expect(await TappayFP.getApplePayPrime('1')).toThrow();
    expect(await TappayFP.getApplePayPrime('1')).toBeUndefined();
  });
  ...
  test('getSamsungPayPrime() test', async () => {
    expect(await TappayFP.getSamsungPayPrime('1', '0', '0', '1')).toMatchObject(
      {
        systemOS,
        tappaySDKVersion: 'mock',
        prime: 'mockPrime',
